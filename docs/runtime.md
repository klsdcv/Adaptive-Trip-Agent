...
`GOOGLE_MAPS_API_KEY`. `APP_DATA_DIR` defaults to `.local`.
...
Current limitations: this entrypoint does not yet start the periodic monitor;
the graph does not yet fetch missing place/route evidence automatically.
